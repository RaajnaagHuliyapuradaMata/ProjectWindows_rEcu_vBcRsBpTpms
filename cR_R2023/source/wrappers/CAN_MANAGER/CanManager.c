...
extern void CANMGR_Init(void)
{
  ...
  if (ucEcuMode == cECUMODE_QUIET)  // wakeup from CAN occured
  {
    ...
    //DemMGR_StartMonitoring(); //RST Debug: don't start monitoring here. Only when ignition gets switched on!
  }

  else if (ucEcuMode == cECUMODE_WAKE)
  {
    #ifdef DEBUG_PRESSURE_ON_KEY
    /* RST: activate CAN in WAKE mode to be able to send app responsen message */
    CANMGR_SetFullCommunicationMode();  // Start ComM, CAN controller and TRCV
    TRCV_Normal();
...
/*****************************************************************************
** Function:    CANMGR_PduGroupInit
** Description: Initialize PDU groups, so that messages are sent/not sent on
**              the bus.
** Parameter:   None
** Return:      None
******************************************************************************/
static void CANMGR_PduGroupInit(void)
{
  ...
  Com_SetIpduGroup(Vector_IPduGrpVctr, ComConf_ComIPduGroup_ComIPduGroup_Rx, TRUE);             /* start all common RX PDUs    */
  Com_SetIpduGroup(Vector_IPduGrpVctr, ComConf_ComIPduGroup_ComIPduGroup_Tx, FALSE);            /* stop all common TX PDUs     */
  Com_SetIpduGroup(Vector_IPduGrpVctr, ComConf_ComIPduGroup_ComIPduGroup_Tx_AppCyclic, FALSE);  /* stop cyclic application PDU */
  Com_SetIpduGroup(Vector_IPduGrpVctr, ComConf_ComIPduGroup_ComIPduGroup_Tx_AppResponse, FALSE);/* stop appl response PDU      */
  Com_IpduGroupControl(Vector_IPduGrpVctr, TRUE);
}

/*****************************************************************************
** Function:    CANMGR_PduGroupStart
** Description: Start a specific PDU group: messages in TX groups will be
**              sent on the bus. Messages in RX groups will be received and
**              processed.
** Parameter:   Com_IpduGroupIdType tGroupId; use the BSW defines
**              e.g. ComConf_ComIPduGroup_ComIPduGroup_Tx_AppCyclic
** Return:      None
******************************************************************************/
extern void CANMGR_PduGroupStart(Com_IpduGroupIdType tGroupId)
{
  /* Activation of TX PDUs only allowed if it's not forbidden by UDS 0x28 CommControl */
