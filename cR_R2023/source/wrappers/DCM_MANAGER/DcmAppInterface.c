...
FUNC(Std_ReturnType,DCM_APPL_CODE) DcmDspData_ApplicationID_ReadFunc (VAR(Dcm_OpStatusType,AUTOMATIC) OpStatus,P2VAR(uint8,AUTOMATIC,DCM_INTERN_DATA) Data)
{
  Data[0] = 1U;
  Data += 1;
  if (VERSION_GetEcuProgramInformationPart1(Data, cSTRING_ECU_PROGRAM_INFO_1_LENGTH, cMETADATA_APP) == FALSE)
  {
    return E_NOT_OK;
  }
  else
...
{
  RdcSt_ReadPressureOnKeyConfig(Data);
...
{
  RdcSt_ReadPressureOnKeyStatus(Data);
  return E_OK;
}
...
{
  RdcSt_WritePressureOnKeyConfig(Data);
