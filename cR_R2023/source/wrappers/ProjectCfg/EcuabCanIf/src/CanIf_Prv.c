...
#include "CanIf_MemMap.h"
VAR(uint8, CANIF_VAR) CanIf_Lok_RxBuffer_tau8[CANIF_CFG_RX_BUFFER_SIZE];
...
#include "CanIf_MemMap.h"
VAR(CanIf_NotifStatusType, CANIF_VAR) CanIf_Lok_RxNotification_taen[CANIF_CFG_NUM_CANRXPDUIDS];
...
VAR(CanIf_Lok_TxBufferStatus_tst, AUTOMATIC)  CanIf_Lok_TxBufferRam_ast[CANIF_TOTAL_TXBUFFERS];
VAR(CanIf_Lok_TxPduStatus_tst, AUTOMATIC)     CanIf_Lok_TxPduRam_ast[CANIF_TOTAL_TXPDUS];
#endif
...
VAR(CanIf_NotifStatusType, CANIF_VAR) CanIf_Lok_TxNotification_aen[CANIF_TOTAL_TXPDUS];
#endif
