...
//extern void DEBUG_SetRssiValues(uint8* ucBuf, uint8 ucAB);
//extern void DEBUG_SetAverage(uint8 avg);
#ifdef DEBUG_LOST_TELEGRAMS
#include "RdcStandstillDebugX.h" //RST Debug Rx
#endif
...
extern void RdcRb_StoreTelegram(struct rfstruct* PS_TelegramData, uint32* PU32_Timestamp)
{
  ...
  else
  {
    ...
    #ifdef DEBUG_LOST_TELEGRAMS
    LTGDEBUG_SetEvent(PS_TelegramData->buffer[0], ulId, cTGFILTER_RBFULL);
    #endif
  }
}
