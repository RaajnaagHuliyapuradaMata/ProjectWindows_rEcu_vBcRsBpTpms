...
#include "SwcApplTpms_DevCanMessages.h"
#include "SwcApplTpms_DevCanMesReqInterfaces.h"
#include "iTpms_Interface.h"
#include "DevCanHandling.h"

#include "global.h"
#include "WallocX.h"    // for ucGetZOMPosOfID(u32) function
#include "SwcApplTpms_NvM_If.h" // for GETbHfIntLatchEE() function
#include "USCS.h"       // for ucGetPSollCS() function
...
static void DCM_PutEventData2Buffer(tsCAN_Message S_EnvDataMultiplex)
{
  ...
  {// don't put it in buffer and don't transmit
  }
}
...
void DCM_DataToTransmitOnCAN(tsCAN_Message* spCAN_Message)
{
  ...
  { // not last system, don't interrupt
    // send system specific multiplexes
    DCM_SystemDataGetFromBuffer(spCAN_Message);
  }
}
