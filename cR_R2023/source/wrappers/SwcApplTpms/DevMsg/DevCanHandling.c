...
static void DCH_ActivateDebugCfg(const tsCAN_Message* sDebugCanMsgCfg)
{
  ...
  { // do nothing, not activated before and not active in current try
  }
}
...
static void DCH_MultiplexMsgUpdate(const tsCAN_Message* sMultiplexCanMsgCfg)
{
  ...
  if( DCH_MULTIPLEX_ENABLED == DCH_bEnableSingleMultiplexConfig )
  {
    ...
    {// do nothing, out of array bounds, not a valid submultiplex value
    }
  }
  else
  {//do nothing, not enabled remain with default
  }
}
