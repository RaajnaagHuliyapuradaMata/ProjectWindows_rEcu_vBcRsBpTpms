...
void Com_InternalMainFunctionTx(Com_MainFuncType TxMainFuncId)
{
   ...
   if(Com_Uninit_Flag == COM_INIT)
   {
        ...
        for( ; idx_TxIpduId_u16 < numOfIpdus; idx_TxIpduId_u16++ )
        {
            ...
            if(Com_CheckTxIPduStatus((Type_SwcServiceCom_tIdPdu)idx_TxIpduId_u16))
            {
...
#define COM_START_SEC_CODE
#include "Com_MemMap.hpp"
boolean Com_IsTxScheduled(Type_SwcServiceCom_tIdPdu ComTxPduId, uint16 ComCallerTaskCycle)
{
   Com_TxIpduRamPtrType        TxIpduRamPtr;
...
#define COM_START_SEC_CODE
#include "Com_MemMap.hpp"
boolean Com_ProvideTxIpduStatus(Type_SwcServiceCom_tIdPdu PduId)
{
   boolean     pduStatus_b;
