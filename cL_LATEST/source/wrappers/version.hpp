...
#define cAPP_DESCRIPTOR_BLOCK_ROM_ADDRESS (void*)0x00077b00U
#define cFBL_DESCRIPTOR_BLOCK_ROM_ADDRESS (void*)0x0001ff00U

#define cAPPLICATION_START_ADDRESS (uint32)0x20000U
#define cAPPLICATION_END_ADDRESS (uint32)0x77BFFU

#define cAPP  0
#define cFBL  1

#define cSW_VERSION_MAJOR  (uint8)1
#define cSW_VERSION_MINOR  (uint8)3
#define cSW_VERSION_MICRO  (uint16)0

#define cLORDSTOWN_VERSION_STRING 'D','E','0','_','_','_','_','_'
...
#define IGNORE_PRESSURE_ON_KEY //Lordstown does not support Pressure on Key. Shutdown will happen in quiet mode after COMM_TIMEOUT, disregarding coast mode or monitoring mode
//#define IGNORE_POS_RESP_ON_STARTUP //Without bootloader no positive response 51 01 shall be sent
#define DEBUG_IGNORE_MESSAGE_AUTHENTICATION //Lordstown has no message authentication
#define USE_CONSTANT_DELAY   // No ABS timestamp for lordstown
...
#define DEBUG_PRESSURE_ON_KEY
#endif

#ifndef DEBUG_IGNORE_MESSAGE_AUTHENTICATION
#define DEBUG_IGNORE_MESSAGE_AUTHENTICATION
#endif

  //#define DEBUG_FAST_SHUTDOWN //Lordstown has no coast mode
...
#define cSTRING_BHSENS_SW_VERSION_LENGTH   4
...
typedef struct{
  ...
  uint8  ucAuxId;
  uint16 ushModeId;
  uint8  aucLordstownSwVersionString[cSTRING_SIZ_LMCSWV];
...
extern uint8 VERSION_GetBhsensSwVersionString(uint8* ucData, uint8 ucLen, uint8 ucApplFbl);
...
extern uint8 VERSION_GetLmcSwVersion        (uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl);
