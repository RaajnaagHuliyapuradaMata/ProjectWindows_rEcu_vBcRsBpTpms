...
void PduR_GF_Tp_TxConfirmation_Func( Type_SwcServiceCom_tIdPdu gwId, Std_ReturnType result){
   PduR_GT_TpPtr      gt;
        ...
        if(gt->TpQueueDepth_u8 > 1u){
            ...
            while(BreakFlag_b){
                ...
                if(SessionPtr->Tx_FifoInstanceUsed_u8 != SessionPtr->Rx_FifoInstanceUsed_u8){
                    Type_SwcServiceCom_stInfoPdu readerPduInfo;
                    Std_ReturnType Ret_val;
                     Type_SwcServiceCom_tLengthPdu TxInstanceLength_l;
                    SessionPtr->Tx_Status = PDUR_TX_IDLE;
                    SessionPtr->readPtr = fifo->begin;
                    TxInstanceLength_l = (Type_SwcServiceCom_tLengthPdu)((SessionPtr->Tx_FifoInstanceUsed_u8)*(SessionPtr->bufferLength));
