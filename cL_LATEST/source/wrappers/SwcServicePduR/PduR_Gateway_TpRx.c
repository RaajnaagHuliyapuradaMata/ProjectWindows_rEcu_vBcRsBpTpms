...
BufReq_ReturnType PduR_GF_Tp_StartOfReception_Func(Type_SwcServiceCom_tIdPdu gwId, const Type_SwcServiceCom_stInfoPdu* info,Type_SwcServiceCom_tLengthPdu TpSduLength, Type_SwcServiceCom_tLengthPdu* bufferSizePtr)
{
...
BufReq_ReturnType PduR_GF_Tp_CopyRxData_Func( Type_SwcServiceCom_tIdPdu gwId, const Type_SwcServiceCom_stInfoPdu* info, Type_SwcServiceCom_tLengthPdu* bufferSizePtr)
{
   ...
   {
        Type_SwcServiceCom_tLengthPdu availableBuffLength_l;
   ...
   if(!((SessionPtr->Tx_Status == PDUR_TX_IDLE) &&
               ...
               (SessionPtr->numRxLength_auo[SessionPtr->Rx_FifoInstanceUsed_u8] == SessionPtr->bufferLength)))))
     {
        ...
        Type_SwcServiceCom_stInfoPdu readerPduInfo;
...
void PduR_GF_Tp_RxIndication_Func( Type_SwcServiceCom_tIdPdu gwId, Std_ReturnType result)
{
   ...
   if(result == E_OK)
   {
         ...
         {
             VAR(Type_SwcServiceCom_stInfoPdu,AUTOMATIC) readerPduInfo;
        ...
        if(gt->TpQueueDepth_u8 > 1u)
        {
            ...
            if(SessionPtr->Rx_FifoInstanceUsed_u8 != gt->TpQueueDepth_u8)
            {
                ...
                Type_SwcServiceCom_tLengthPdu RxInstanceLength_l;
                RxInstanceLength_l = (Type_SwcServiceCom_tLengthPdu)((SessionPtr->Rx_FifoInstanceUsed_u8)*(SessionPtr->bufferLength));
...
#endif
      {
         ...
         PduR_ResetPduR_FIFO_Tp_dynamic_Structure(
            SessionPtr
         );
      }
   }
