...
typedef struct{
   ...
   Type_SwcServiceCom_tLengthPdu                        numRxLength_auo[PDUR_MAX_QUEUEDEPTH];
   Type_SwcServiceCom_tLengthPdu                        numTxLength;
   Type_SwcServiceCom_tLengthPdu                        SduLength_auo[PDUR_MAX_QUEUEDEPTH];
...
typedef struct{
   P2VAR(uint8, TYPEDEF, PDUR_VAR) readPtr;
   P2VAR(uint8, TYPEDEF, PDUR_VAR) writePtr;
   Type_SwcServiceCom_tLengthPdu   actLen[PDUR_FIFO_IF_MAXSIZE];
   uint8                           used;
   uint8                           txConfPending;
   uint8                           readIndex;
   uint8                           writeIndex;
