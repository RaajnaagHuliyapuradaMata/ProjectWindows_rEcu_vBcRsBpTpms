...
const Type_CfgSwcServiceDcm_st Dcm_Config[DCM_PRV_MAXNUM_OF_CONFIG] = {
      {DCM_CFG_CONFIGSET1}
   ,  {DCM_CFG_CONFIGSET2}
   ,  {DCM_CFG_CONFIGSET3}
   ,  {DCM_CFG_CONFIGSET4}
...
static FUNC(void, SWCSERVICEDCM_CODE) Dcm_Lok_IOCBIIinit(void){
   ...
   {
        for(idxSig_u16=0; idxSig_u16<CfgSwcServiceDcm_astListDid[idxDid_u16].nrSig_u16; idxSig_u16++)
        {

            if((CfgSwcServiceDcm_astListDid[idxDid_u16].pcstDidExtended->dataSessBitMask_u32!=0x00u) && (CfgSwcServiceDcm_castListInfoDidWrite[Dcm_DspDataInfo_st[CfgSwcServiceDcm_astListDid[idxDid_u16].pcstSignalDid[idxSig_u16].idxDcmDspDatainfo_u16].idxDcmDspControlInfo_u16].idxDcmDspIocbiInfo_u16>0))
            {
                break;
            }
        }
        if(idxSig_u16<CfgSwcServiceDcm_astListDid[idxDid_u16].nrSig_u16)
        {
            ...
            if((TRUE == CfgSwcServiceDcm_astListDid[idxDid_u16].AtomicorNewSRCommunication_b) && \
               ((USE_ATOMIC_SENDER_RECEIVER_INTERFACE == CfgSwcServiceDcm_astListDid[idxDid_u16].didUsePort_u8) || (USE_ATOMIC_SENDER_RECEIVER_INTERFACE_AS_SERVICE == CfgSwcServiceDcm_astListDid[idxDid_u16].didUsePort_u8)))
            {

                (void)(*(IOControlrequest_pfct)(CfgSwcServiceDcm_astListDid[idxDid_u16].ioControlRequest_cpv))(DCM_IOCBI_INIT,NULL_PTR,DCM_VALUE_NULL,DCM_VALUE_NULL,DCM_INITIAL,NULL_PTR);
            }
#endif
        }
   }
