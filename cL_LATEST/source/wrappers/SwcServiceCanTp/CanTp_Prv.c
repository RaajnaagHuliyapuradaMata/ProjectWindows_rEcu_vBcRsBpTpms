...
static void CanTp_Lok_GetTxBuffer(
   ...
   if(
      ...
      <  Context->CsTicks
        ...
        if(
              CanTp_Channel[Context->ChannelId].TxBufferStatus
           != CANTP_BUFFER_AVAILABLE
        ){
            switch(
   ...
   if(
      ...
      if(
         ...
         if(
            ...
            && (Connection->AddressFormatId != CANTP_NORMALFIXED)
         ){
...
static void SwcApplCanTp_fptrSubState_TxSendConsecutiveFrame(
   ...
   if(
      ...
      CanTp_Lok_GetTxBuffer(
            &Context
         ,  &SduInfo
      );

      if(
            (Channel->TxBufferStatus == CANTP_BUFFER_AVAILABLE)
         && (
