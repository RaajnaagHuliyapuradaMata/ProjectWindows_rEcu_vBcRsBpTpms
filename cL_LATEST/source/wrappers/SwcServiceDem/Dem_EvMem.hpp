...
DEM_ARRAY_DECLARE_CONST(Dem_EvMemMapOrigin2IdType, Dem_EvMemMapOrigin2Id, 6);
...
DEM_INLINE void Dem_EvMemSetEventFailedAllMem(Dem_EventIdType EventId, const uint8 *EnvData)
{
   ...
   if(Dem_EvtParam_GetIsEventDestSecondary(EventId))
   {
...
DEM_INLINE uint16_least Dem_EvMemGetMemIdForDTCOrigin(Dem_DTCOriginType DTCOrigin)
{
   ...
   switch(DTCOrigin)
   {
        case DEM_DTC_ORIGIN_PRIMARY_MEMORY:
            returnValue = DEM_CFG_EVMEM_MEMID_PRIMARY;
            break;

#if(DEM_CFG_MAX_NUMBER_EVENT_ENTRY_SECONDARY > 0)
        case DEM_DTC_ORIGIN_SECONDARY_MEMORY:
            returnValue = DEM_CFG_EVMEM_MEMID_SECONDARY;
            break;
#endif

#if(DEM_CFG_MAX_NUMBER_EVENT_ENTRY_MIRROR > 0)
        case DEM_DTC_ORIGIN_MIRROR_MEMORY:
            returnValue = DEM_CFG_EVMEM_MEMID_MIRROR;
            break;
#elif DEM_CFG_EVMEM_SHADOW_MEMORY_SUPPORTED
        case DEM_DTC_ORIGIN_MIRROR_MEMORY:
            returnValue = DEM_CFG_EVMEM_MEMID_SHADOW;
            break;
#endif

        default:
            returnValue = DEM_EVMEM_INVALID_MEMID;
            break;
   }
    return returnValue;
}
