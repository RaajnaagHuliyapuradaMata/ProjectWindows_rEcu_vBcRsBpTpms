...
DEM_INLINE Std_ReturnType Dem_EnvFFRetrieve(uint8 freezeFrameId
   ,  	uint8 RecNum
   ,  	uint8* dest
   ,  	uint16* bufsize
   ,  	const uint8* src
   ,  	const Dem_InternalEnvData* internalEnvData)
{
   ...
   if(*bufsize < 2)
   {
       returnValue = DEM_BUFFER_TOO_SMALL;
   }
   else{
        *writepos = RecNum;
        writepos++;
...
DEM_INLINE Std_ReturnType Dem_EnvFFGetSize(uint8 freezeFrameId, uint16* size)
{
   uint16_least i;
   *size = 2;
