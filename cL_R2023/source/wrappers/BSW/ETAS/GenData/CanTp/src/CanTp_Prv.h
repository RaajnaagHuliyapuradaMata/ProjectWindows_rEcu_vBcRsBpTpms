...
LOCAL_INLINE void CanTp_Lok_TxConfirmation_ArrayInit(CanTp_ChannelIdType *ArrayPtr, PduIdType NoOfTxPdus,
...
LOCAL_INLINE void CanTp_Lok_CanIfTransmit(const CanTp_TxContextType *Context, const PduInfoType *CanIfTxInfoPtr)
{
    ...
    if(CanTp_Lok_GetBit(Context->BitFields, CANTP_TRIGGER_TRANSMIT_ON))
    {
...
LOCAL_INLINE void CanTp_Lok_PageConfirmation(const CanTp_TxContextType *Context)
{
    PduInfoType SduInfo;
    PduLengthType RemBufSize;
    uint8 SduBuffer[CANTP_MAX_NPDU_LENGTH];
...
LOCAL_INLINE void CanTp_Lok_LengthError(const CanTp_RxContextType *Context)
{
  ...
  if(Context->IsPaddingOn)
    ...
    if((Context->IsPaddingOn) && (Context->FrameType!=CANTP_FIRST_FRAME))
    {
