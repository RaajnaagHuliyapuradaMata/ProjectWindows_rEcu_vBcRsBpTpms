...
Std_ReturnType Dem_DltGetMostRecentFreezeFrameRecordData(Dem_EventIdType EventId, uint8* DestBuffer, uint16* BufSize)
{
    ...
    if (Dem_EvMemGetReaderCopyOfEventFromEventMemory(&ReaderCopy, EventId) == E_OK)
    {
        ...
        returnValue = Dem_EnvRetrieveRawFF(EventId, DestBuffer, BufSize, (ffIndex - 1u), Dem_EvMemGetEventMemDataByPtr(&ReaderCopy), &ReaderCopy);
    }
    else
	{
...
)
{
	...
	DEM_ENTRY_CONDITION_CHECK_NOT_NULL_PTR_WITHEVENTID(EventId,DestBuffer,DEM_DET_APIID_GETEVENTEXTENDEDDATARECORD,E_NOT_OK);
	DEM_ENTRY_CONDITION_CHECK_NOT_NULL_PTR_WITHEVENTID(EventId,BufSize,DEM_DET_APIID_GETEVENTEXTENDEDDATARECORD,E_NOT_OK);
...
)
{
	...
	DEM_ENTRY_CONDITION_CHECK_NOT_NULL_PTR_WITHEVENTID(EventId,DestBuffer,DEM_DET_APIID_GETEVENTFREEZEFRAMEDATA,E_NOT_OK);
	DEM_ENTRY_CONDITION_CHECK_NOT_NULL_PTR_WITHEVENTID(EventId,BufSize,DEM_DET_APIID_GETEVENTFREEZEFRAMEDATA,E_NOT_OK);
   ...
   if (Dem_EvMemGetReaderCopyOfEventFromEventMemory(&ReaderCopy, EventId) == E_OK)
   {
      ...
      else if (RecordNumber == 0x00u)
      {
         ...
         if (((DataId & 0xFF00u) == 0xF400u) && (rba_DemObdBasic_FF_IsObdFFAvailable(&ReaderCopy)))
         {
		...
		{
			DEM_DET(DEM_DET_APIID_GETEVENTFREEZEFRAMEDATA, 0,EventId);
			return E_NOT_OK;
		}
		else
		{
...
#endif
			{
			    if( Dem_EnvRetrieveRawDid(EventId, DestBuffer, BufSize, (ffIndex), DataId, Dem_EvMemGetEventMemDataByPtr(&ReaderCopy), &ReaderCopy) == FALSE )
				{
					returnValue = E_NOT_OK;
				}
				else
