...
#if (DEM_CFG_OBD == DEM_CFG_OBD_ON)
	        DtcId = Dem_DtcIdFromEventId(EventId);
	        if ((Dem_isDtcIdValid(DtcId)) && (Dem_Cfg_Dtc_GetKind(DtcId) == DEM_DTC_KIND_EMISSION_REL_DTCS))
	        {
	            Dem_EvMemGenReportEvent(EventId,DEM_FIRST_FAILED_OBD_DTC,DEM_MOST_RECENT_FAILED_OBD_DTC);
	        }
#endif
	        DEM_EXITLOCK_MON();
	    }
