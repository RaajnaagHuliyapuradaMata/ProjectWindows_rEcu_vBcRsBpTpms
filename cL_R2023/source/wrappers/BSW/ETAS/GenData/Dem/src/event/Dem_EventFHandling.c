...
#include "Dem_Lok_CallEvtStChngdCbk.h"
#include "Dem_Obd.h"
        ...
        Dem_DebugDataType debug0, Dem_DebugDataType debug1)
{
    ...
    if(!isInBuffer)
    {
        if(!Dem_RingBuffer__insert(&Dem_ErrorQueue_Handler, &loc))
        {
            ...
            DEM_DET(DEM_DET_APIID_REPORERRORSTATUSQUEUE,Dem_ErrorQueueControl.overflowcounter,EventId);
        ...
        Dem_DebugDataType debug0, Dem_DebugDataType debug1)
{
    ...
    Dem_RingBufferIndex__ISINBUFFER(Dem_UnrobustErrorQueue,Dem_CompareFuncForEvents, EventId, isInBuffer, indx);
    if(!isInBuffer)
    {
        if(!Dem_RingBuffer__insert(&Dem_UnrobustErrorQueue_Handler, &indx))
        {
            Dem_UnrobustQueueControl.overflowcounter++;
            DEM_DET(DEM_DET_APIID_REPORTUNROBUSTQUEUEOVERFLOW,Dem_UnrobustQueueControl.overflowcounter,EventId);
            islocationvalid = FALSE;
        }
    }
    ...
    if(islocationvalid)
    {
        Dem_UnrobustErrorQueue[indx].EventId = EventId;
        Dem_UnrobustErrorQueue[indx].EventStatus = EventStatus;
#if(DEM_CFG_DEBUGDATA_BEFOREINIT == DEM_CFG_DEBUGDATA_BEFOREINIT_ON)
        Dem_UnrobustErrorQueue[indx].debug0 = debug0;
        Dem_UnrobustErrorQueue[indx].debug1 = debug1;
#else
        DEM_UNUSED_PARAM(debug0);
        DEM_UNUSED_PARAM(debug1);
#endif
...
static void Dem_ReportErrorStatusDequeue(Dem_ErrorQueueControlType *queueControl, Dem_RingBuffer *queueHandler,
        DEM_ARRAY_FUNCPARAM(Dem_ErrorQueueType ,queue))
{
              ...
              Dem_IsEventReportingEnabledByDtcSetting(EventId))
      {
         ...
         if (debAction != DEM_DEBACTION_NOOP)
         {
           ...
           if((debAction & allowEnqueueUnrobust) == allowEnqueueUnrobust)
           {
               ...
               if(Dem_UnrobustQueueControl.isQueueEnabled)
               {
                   Dem_UnrobustEventEnqueue(EventId, EventStatus, debug0, debug1);
               }
               DEM_EXITLOCK_MON();
           }
#endif
