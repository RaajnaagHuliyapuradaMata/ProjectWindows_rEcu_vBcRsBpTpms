...
#define DEM_ASSERT_EVMEMREADERCOPIES_ISLOCKED()    do{if(Dem_LibGetParamBool(DEM_CFG_EVMEM_READ_FROM_DIFFERENT_TASK)) { DEM_ASSERT_ISLOCKED(); }}while(0)
...
DEM_INLINE uint8 Dem_EvMemGetEventMemFreezeFrameCounterByPtr(const Dem_EvMemEventMemoryType *EventMemory)
{
   DEM_EVMEM_USEVAR(EventMemory);
...
DEM_INLINE uint8 Dem_EvMemGetEventMemFreezeFrameCounter(uint16_least LocId)
{
   return Dem_EvMemGetEventMemFreezeFrameCounterByPtr(&Dem_EvMemEventMemory[LocId]);
}

DEM_INLINE void Dem_EvMemSetEventMemLocFreezeFrameCounter(uint16_least LocId, uint8 FreezeFrameCounter)
{
   DEM_EVMEM_USEVAR(LocId);
