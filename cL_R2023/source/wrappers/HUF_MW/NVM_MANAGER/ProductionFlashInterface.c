...
#include "ProductionFlashInterface.h"
#include "Fls.h"
#include "r_fdl.h"
#include "gpioX.h"

#ifdef _RST_VS_EDITOR_SUPPORT_
  //just to get an error free view in visual studio -> ms doesn't know ".production_data"
...
uint8 PRODFLASH_GetCustomerHwPartNumber(uint8* ucData)
{
  ...
  if (GPIO_GetFrequencySetting() == cFREQUENCY_433)
  {
    ...
    pHwVersionNumber = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index2_CustomerHwPartNumber433;
  }
  else
  {
    ucSize = sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index3_CustomerHwPartNumber315);
    pHwVersionNumber = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index3_CustomerHwPartNumber315;
  }

  for(ucCounter = 0U; ucCounter < ucSize; ucCounter++)
  {
    ucData[ucCounter] = pHwVersionNumber[ucCounter];
  }
...
uint8 PRODFLASH_GetApplicationChecksum(uint8* ucData)
{
	uint8 ucCounter;
	uint8 ucSize;

	ucSize = sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index31_ApplicationChecksum);

	for (ucCounter = 0U; ucCounter < ucSize; ucCounter++)
	{
		ucData[ucCounter] = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index31_ApplicationChecksum[ucCounter];
	}

	return ucSize;
}

uint8 PRODFLASH_GetBhsensProductionDate(uint8* ucData)
{
  uint8 ucCounter;
  uint8 ucSize;

  ucSize = sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index10_ProductionDay);

  for (ucCounter = 0U; ucCounter < ucSize; ucCounter++)
  {
    ucData[ucCounter] = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index10_ProductionDay[ucCounter];
  }

  return ucSize;
}

uint8 PRODFLASH_GetBhsensHardwareNumber(uint8* ucData)
{
  uint8 ucCounter;
  uint8 ucSize;

  ucSize = sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index4_HufSpecificHw433);

  for (ucCounter = 0U; ucCounter < ucSize; ucCounter++)
  {
    ucData[ucCounter] = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index4_HufSpecificHw433[ucCounter];
  }

  return ucSize;
}

uint8 PRODFLASH_GetBhsensHwVersionInfo(uint8* ucData)
{
  uint8 ucCounter;
  uint8 ucSize;

  ucSize = sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index6_HardwareVersionInfo);

  for (ucCounter = 0U; ucCounter < ucSize; ucCounter++)
  {
    ucData[ucCounter] = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index6_HardwareVersionInfo[ucCounter];
  }

  return ucSize;
}

uint8 PRODFLASH_GetProgrammingDate(uint8* ucData)
{
	uint8 ucCounter;
	uint8 ucSize;
	uint8 i;
	...
	ucSize += sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index13_RivianApplSwFpProgrammingDate);
	for (i = 0, ucCounter = ucCounter; ucCounter < ucSize; i++, ucCounter++)
	{
		ucData[i] = DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index13_RivianApplSwFpProgrammingDate[ucCounter];
	}	
...
void PRODFLASH_PutProgrammingDate(const uint8* ucData)
{
	PRODFLASH__WriteProductionDataMember(DCM__E_RIVIAN_APPL_SWFP_PROGRAMMING_DATE, &ucData[0], sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index13_RivianApplSwFpProgrammingDate));
}

void PRODFLASH_PutCustomerEcuSerialNumber(const uint8* ucData)
{
	PRODFLASH__WriteProductionDataMember(DCM__E_CUSTOMER_ECU_SERIAL_NUMBER, &ucData[0], sizeof(DCM__UN_ProductionDataFlash.Struct_Split_Production_Data.AU8_Index11_CustomerECUSerialNumber));
}
