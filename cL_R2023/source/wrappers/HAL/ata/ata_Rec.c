...
//uint8 ushRxCounter_CrcOk = 0;
//void Debug_GetAtaRecCounters(uint8* Ct1_Systemfehler, uint8* Ct2_TgLen, uint8* Ct3_Crc)
//{
//  *Ct1_Systemfehler = ushSystemFehler;
//  *Ct2_TgLen = ushRxCounter_TgLenOk;
//  *Ct3_Crc = ushRxCounter_CrcOk;
//}
...
void CheckAtaRec(void)
{
  ...
  )  // cfg = activated one && OK events on path B or A
  {
    ...
    if(rf.level > (uint8)3)
    {
      ...
      ui8MessageLen = GetAtaTelegramLength(rf.buffer[0]);
      ...
      {
        //ushRxCounter_CrcOk++; //RST Debug Rx Crc failed!
      }
    }
    else
    {
