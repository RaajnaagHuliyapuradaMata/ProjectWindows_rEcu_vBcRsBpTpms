...
typedef Std_ReturnType (*Type_SwcServiceCanTp_fptrCreateFrame)(CanTp_ChannelIdType ChannelId, PduInfoType *PduInfoPtr);
...
typedef void (*Type_SwcServiceCanTp_fptrProcessFrame)(const CanTp_RxContextType *Context, const PduInfoType *PduInfoPtr);
...
typedef void (*Type_SwcServicePduR_fptrConfirmationTpLo)(PduIdType id, Std_ReturnType result);
...
LOCAL_INLINE PduLengthType CanTp_Lok_GetTxBlockCfs(
   CanTp_ChannelIdType ChannelId
){
   uint8 PayLoadLength;
...
LOCAL_INLINE PduLengthType CanTp_Lok_GetRxBlockCfs(
   CanTp_ChannelIdType ChannelId
){
   uint8 PayLoadLength;
...
LOCAL_INLINE void CanTp_Lok_SetRxBlockInfo(
            CanTp_ChannelIdType ChannelId
   ,  const PduLengthType       RemBufSize
){
   uint8 BSMax;
   PduLengthType BlockCfsRemaining;
   ...
   PduLengthType OneFramePayloadLength = CanTp_Lok_GetRxDl(ChannelId) - CanTp_AddressSize[Connection->AddressFormatId] - CANTP_CF_PCISIZE;
#if(CANTP_DYNAMIC_FC_SUPPORT != CANTP_ON)
   if(Channel->STminTicks == CANTP_INVALID_STMIN_TICKS)
#endif
   {
