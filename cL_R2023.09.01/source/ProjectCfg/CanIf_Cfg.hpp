...
typedef struct{
   P2FUNC(void, TYPEDEF, CanIfRxPduUserRxIndicationUser)(
             VAR(PduIdType,         AUTOMATIC               ) RxPduTargetId_t
      ,  P2CONST(CanIf_PduInfoType, TYPEDEF, CANIF_APPL_DATA) ULPduInfoTypUSER_tst
   );
}CanIf_RxUSERCbk_Prototype;

typedef struct{
   P2FUNC(void, CANIF_APPL_CODE, UserTxConfirmation)(PduIdType TxPduTargetPduId);
