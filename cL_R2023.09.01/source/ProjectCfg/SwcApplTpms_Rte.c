...
#if defined(BUILD_LORDSTOWN)
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrFL <= cAbsOverflowValue) ? TRUE : FALSE);
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrFR <= cAbsOverflowValue) ? TRUE : FALSE);
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrRL <= cAbsOverflowValue) ? TRUE : FALSE);
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrRR <= cAbsOverflowValue) ? TRUE : FALSE);
    #elif defined (BUILD_RIVIAN)
    // RST 21.10.2021: Rivian has a valid range of pulses 0..255, so the invalid should be 0xffff
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrFL <= 0xff) ? TRUE : FALSE);
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrFR <= 0xff) ? TRUE : FALSE);
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrRL <= 0xff) ? TRUE : FALSE);
   ucReceiveValid &= ((spRxEnvAbsDataIn->uiPulseCtrRR <= 0xff) ? TRUE : FALSE);
#else
...
static uint8 u8CheckSameTG(const uint8 *u8CurrentRFFrame, uint32 u32_TimeStamp)
{
  ...
  { // ID was not found
   ...
   else{ // DO NOT TX, time is smaller than TIME_TO_ALLOW_SAME_TGms
      Return = 1;
   }
  }
