...
FUNC(void, COM_CODE) Com_InternalMainFunctionTx( VAR(Com_MainFuncType,AUTOMATIC) TxMainFuncId )
{
        ...
        )
   {
        ...
        for( Idx_Pdu_ui = StartIPduId; Idx_Pdu_ui < EndIPduId; Idx_Pdu_ui++ )
        {
            ...
            if(Com_CheckTxIPduStatus((Type_SwcServiceCom_tIdPdu)Idx_Pdu_ui))
            {
...
#endif
        {
            Com_Lok_SendIpdu((Type_SwcServiceCom_tIdPdu)TxPduId,sendIpduFlag_u16);
        }
   }
}
...
#define COM_START_SEC_CODE
#include "Com_MemMap.hpp"
FUNC(boolean, COM_CODE) Com_IsTxScheduled( VAR(Type_SwcServiceCom_tIdPdu, AUTOMATIC) ComTxPduId
...
FUNC(boolean,COM_CODE) Com_ProvideTxIpduStatus(VAR(Type_SwcServiceCom_tIdPdu, AUTOMATIC) PduId)
{
   ...
   if(PduId < COM_GET_NUM_TX_IPDU)
   {
        ...
        pduStatus_b = (Com_CheckTxIPduStatus((Type_SwcServiceCom_tIdPdu)PduId));
   }
   else{
